...
LOG_FILE = os.path.join(PROJECT_DIR, "test_results_scenario1.log")
results  = []
...
print(f"\n{BOLD}{CYAN}{'═'*64}{RESET}")
print(f"{BOLD}{CYAN}  Scenario 1 — Secure Direct Communication{RESET}")
...
try:
    ...
    record("TC-SC1-001", "Send short text message", passed)
except Exception:
    record("TC-SC1-001", "Send short text message", False)

try:
    long_msg = ("A" * 500).encode()
    received, _ = run_roundtrip(long_msg, "text", PRIV_PEM, PUB_PEM)
    record("TC-SC1-002", "500-char message", received == long_msg)
except:
    record("TC-SC1-002", "500-char message", False)

try:
    special = "مرحبا Test ñ ü".encode("utf-8")
    received, _ = run_roundtrip(special, "text", PRIV_PEM, PUB_PEM)
    record("TC-SC1-003", "Unicode message", received == special)
except:
    record("TC-SC1-003", "Unicode message", False)

try:
    payload = b"audio" * 1000
    received, _ = run_roundtrip(payload, "audio", PRIV_PEM, PUB_PEM)
    record("TC-SC1-004", "Audio simulation", received == payload)
except:
    record("TC-SC1-004", "Audio simulation", False)

try:
    payload = b"integrity"
    pkt = hybrid_encrypt(PUB_PEM, payload)
    plain = hybrid_decrypt(PRIV_PEM, pkt)
    record("TC-SC1-005", "SHA-256 check", plain == payload)
except:
    record("TC-SC1-005", "SHA-256 check", False)
...
try:
    ...
    for m in msgs:
        ...
        received.append(r)
    record("TC-SC1-006", "Multiple messages", received == msgs)
except:
    record("TC-SC1-006", "Multiple messages", False)
...
try:
    ...
    record("TC-SC1-007", "Key persistence", plain == b"test")
except:
    record("TC-SC1-007", "Key persistence", False)

try:
    open("/tmp/not_exist.pem")
    record("TC-SC1-008", "Missing key", False)
except:
    record("TC-SC1-008", "Missing key", True)
